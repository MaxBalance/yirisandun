...
.controller('OrderCtrl',
    ['$scope', '$http','$ionicLoading','$ionicHistory','Address','Ds','Login','$ionicModal','$state','$ionicPopup','ProductDetail','$stateParams','$filter','Order','Pay',
        function($scope, $http,$ionicLoading,$ionicHistory,Address,Ds,Login,$ionicModal,$state,$ionicPopup,ProductDetail,$stateParams,$filter,Order,Pay) {
            ...
            $scope._add_address = function (address) {
                ...
                }else if(address.place == undefined){
                    //alert("请选择自提点!");
                    var alertPopup = $ionicPopup.alert({
                        title:'请选择自提点!',
                        okType:'button-balanced'
            ...
            $scope._modify_address = function (addressList) {
                if(addressList.area == undefined){
                ...
                }else if(addressList.place == undefined){
                    //alert("请选择自提点!");
                    var alertPopup = $ionicPopup.alert({
                        title:'请选择自提点!',
                        okType:'button-balanced'
                    ...
                    Address.address_modify(addressList,userid);
                }
            }
                    ...
                    if(Order.state == 1){
                        ...
                        confirmPopup.then(function(res) {
                            if(res) {
                                ...
                                cordova.plugins.Pay.alipay([orderInfo.orderid,'一日三顿','一日三顿','0.01']);
                            } else {
                                $state.go('all_order');
                            }
                        });
                ...
                myPopup.then(function(res) {
                        if(res){
                            ...
                            confirmPopup.then(function(res) {
                                if(res) {
                                    //TODO: 修改支付金额  orderInfo.total
                                    cordova.plugins.Pay.alipay([order.orderid,'一日三顿','一日三顿','0.01']);
                                } else {
                                    $state.go('all_order');
                                }
                            });
